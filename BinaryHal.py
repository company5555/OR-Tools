...
if status == pywraplp.Solver.OPTIMAL:
    ...
    for urun in urunler:
        if y[urun].solution_value() > 0:  # Sadece üretilmesi seçilen ürünleri göster
            urun_toplam_adet = 0
    ...
    for uretici in ureticiler:
        if z[uretici].solution_value() > 0:
            print(f"Üretici {uretici} kullanılıyor")
        else:
            print(f"Üretici {uretici} kullanılmıyor")
